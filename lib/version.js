...
module.exports = function(api, parent, verOpts) {
    ...
    ver.findRoute = function(routeName, type) {
        if (type == "docs") {
            if (ver.docRoutes[routeName]) {
                return ver.docRoutes[routeName];
            } else if (parent) {
                return parent.findRoute(routeName, type);
            } else {
                return undefined;
            }
        } else {
            for (var i = 0; i < ver.apiRoutes.length; i++) {
                var match = ver.apiRoutes[i].test(routeName, false);
                if (match) {
                    return ver.apiRoutes[i];
                }
            }
    ...
    ver.router = function(routeName, type, req, res, next) {
        ...
        var route = ver.findRoute(routeName, type);
        if (route && type == "docs") {
            route(req, res, next);
        } else if (route) {
            req.params = route.test(routeName, true);
    ...
    var addEndpoint = function(method, opts) {
        ...
        apiObj.test = function(value, createParams) { //https://github.com/visionmedia/express/blob/288176bbc9bd66f6844f13fdb72d2a1928545bc3/lib/router/route.js
            //returns null if not a match, returns a params object if it is.
            var parts = value.match(apiObj.match);

            if (parts && createParams) {
                var params = {};
        ...
        ver.docRoutes[docsRoute] = ver.docRoutes[docsRoute] ? ver.docRoutes[docsRoute].add(docsObj) : doctor(ver, docsRoute, docsObj);
    }
