...
module.exports = function(api, parent, verOpts) {
    ...
    ver.addType = function(name, example, validator) {
        ver.types[name] = type(name, example, validator);
    }

    ver.removeType = function(name) {
        ver.types[name] = type(name, false, function() {
            return false
        });
    }
    ...
    var addEndpoint = function(method, opts) {
        ...
        ver.docRoutes[docsRoute] = ver.docRoutes[docsRoute] ? ver.docRoutes[docsRoute].add(docsObject) : doctor(docsObj);
    }
